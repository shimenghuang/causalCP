...
get_all_est_one_seed_tmp <- function(output_prefix,
  ...
  dpdu_df_seed <- vector("list", length(reps))
  for (kk in reps) {
    ...
    if (!is.null(res_all_lst$dpdu_all)) {
      dpdu_ests <- res_all_lst$dpdu_all[[1]]
    } else {
      dpdu_ests <- integer(0)
    }
    if (length(dpdu_ests) == 0) dpdu_ests <- n + 1
    if (!is.null(res_all_lst$dpdu_time)) {
      dpdu_time_val <- res_all_lst$dpdu_time
    } else {
      dpdu_time_val <- NA_real_
    }
    dpdu_df_seed[[kk]] <- data.frame(est_value = as.double(dpdu_ests),
                                     cp_scale = nccp_scale,
                                     n = n,
                                     rep = kk,
                                     seed_num = seed_num) %>%
      dplyr::select(seed_num, cp_scale, n, rep, est_value) %>%
      dplyr::mutate(time = dpdu_time_val)
  }
  ...
  dpdu_df_seed <- do.call(rbind, dpdu_df_seed)
...
get_est <- function(output_prefix, fixed_sign, cp_scale, seed_num, reps, ns,
  ...
  for (kk in reps) {
    res_lst <- readRDS(paste0(output_prefix,
                              ...
                              "_rep", kk,
                              ".rds"))
    res_df_lst <- lapply(1:length(methods), \(rr) {
      res <- res_lst[[rr]]
    ...
    time_df <- lapply((length(methods)+1):length(res_lst), \(rr) {
      res <- res_lst[[rr]]
      df <- data.frame(n = ns, time = res, method = methods[rr-length(methods)])
    }) %>%
      dplyr::bind_rows()
    time_all[[kk]] <- time_df
  }
...
res_df <- lapply(ns, \(n) {
  ...
  grid_pts <- ceiling(seq(from = min_dis + 1, to = n - min_dis + 1, by = min_dis))
  output_prefix <- paste0("./inst/output/ccpl_exp1-1",
                          "_allmethods",
...
# plot the estimates
methods <- c("CSL", "OracleCandidates", "BPCandidates", "Breakpoints", "DPDU")
p <- res_df %>%
...
# # ---- Figure 8 ----
# output_prefix <- "./inst/output/ccpl_exp3-1_allmethods"
